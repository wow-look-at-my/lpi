...
- refDone = `weightDone * RefDuration`, refLeft = `max(1 - weightDone - skipped, 0) * RefDuration`
- pace = `elapsed_current / refDone`
- paceApplied = `1 + (pace - 1) * Progress`
- ETA = `refLeft * paceApplied`, kind `"pace"`

This applies only when elapsed time is known, `Progress >= 0.02`, matched lines >= 5, and `refDone > 0`.

The pace denominator is `weightDone`, the raw matched weight. It is never `Progress`. `Progress` is renormalized over retired work. It is therefore a share of what THIS run does rather than a share of the reference clock. Dividing elapsed by it credits the run with reference time it never covered. The run then reads faster than it ran by the factor it skipped. An incremental build skips the most. So the run whose pace matters most is the run whose pace was worst. The remainder had the mirror error. `refLeft` counts only reference time this run still owes. `(1 - Progress) * RefDuration` counted retired work a second time. The two errors cancel in their product. That is why the ETA was right while the reported pace was not.
...
Fallback when elapsed is unknown but `RefDuration > 0` and `Progress >= 0.001`: ETA = `refLeft`, kind `"ref-pace"`.
