...
// Recorder digests a completed run into a Run. Feed it that run's values in
...
// NewRecorder returns a Recorder reading values with tok. source labels the
// recording in model listings.
func NewRecorder[T any](source string, tok Tokenizer[T]) *Recorder[T] {
	return &Recorder[T]{tok: tok, dig: model.NewDigester(source, nil)}
}

// Observe records v, emitted at at. An unset at inherits the previous clock.
func (r *Recorder[T]) Observe(v T, at time.Time) {
	if tok, ok := r.tok(v); ok {
		r.dig.Token(uint64(tok), at)
	}
}

// Finish converts the recording into a Run, and fails on a recording too short to place anything.
func (r *Recorder[T]) Finish() (*Run, error) { return r.dig.Finish() }
